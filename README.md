...
- Inputs: 
-- data_dir: /path/to/your_data/
-- data_name: data_name.csv/data_name.csv.gz (rows:= genes, columns:= samples/cells)
-- percentages: "5,10,20" # means top dense 5%, 10%, 20%, ... samples

- Outputs:
-- data_name_dense_genes123_samples456.csv.gz (rows:= genes, columns:= samples/cells), default saved in `data_dir`
...
- Outputs:
-- data_name_imputed.csv.gz (rows:= genes, columns:= samples/cells), default saved in `data_dir`
